...
;(function(w,d) { 
	if ( Modernizr.csstransforms ) {
		...
		p = d[g]('gprev'),
		n = d[g]('gnext'),
		gallery = new Swipe(d[g]('slider'), {
			speed: 700,
			auto: 2000,
			callback: function(event, index, elem) { 
				var l = gallery.slides.length-1
				n.classList[(index == l)?'add':'remove']('hide');
				p.classList[(index == 0)?'add':'remove']('hide');
			}
		})
		p[a]('click', function() { gallery.prev() }, false);
		n[a]('click', function() { gallery.next() }, false);
	}
}(window,document));